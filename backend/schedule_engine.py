...
def run_scheduling(flights_df: pd.DataFrame, model: RandomForestRegressor, date: str) -> pd.DataFrame:
    ...
    Optimized to batch-predict all hours for each flight.
    """
    scheduled_flights = []
    ...
    # daily_flights = flights_df[flights_df["FL_DATE"] == selected_date]
    daily_flights = flights_df[flights_df['FL_DATE'] == date]

    if daily_flights.empty:
        print(f"❌ No flights found for {selected_date}")
        return pd.DataFrame()
    ...
    for _, flight in daily_flights.iterrows():
        ...
        for hour in range(5, 23):  # 5 AM to 10 PM
            ...
            dt = pd.to_datetime(flight["FL_DATE"])
        ...
        missing = set(model.feature_names_in_) - set(test_df_full.columns)
        for col in missing:
            test_df_full[col] = 0

        # Match model input columns
        test_df = test_df_full[model.feature_names_in_]

        # Predict all 18 hour delays
        predicted_delays = model.predict(test_df)

        # Find best hour
        min_index = np.argmin(predicted_delays)
        best_hour = flight["CRS_DEP_TIME"]
        best_delay = predicted_delays[min_index]
        ...
        scheduled_flights.append({
            ...
            "CRS_ELAPSED_TIME": flight.get("CRS_ELAPSED_TIME", 60),
            "Best_Hour": best_hour,
            "Predicted_Delay": round(float(best_delay), 2)
        })
